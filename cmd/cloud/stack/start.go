...
// startCmd represents the list command
...
func init() {
	startCmd.Flags().StringP("server", "S", "", "The server name to fetch the stack list from")
	startCmd.MarkFlagRequired("server")

	startCmd.Flags().StringP("stack", "s", "", "The stack to start")
	startCmd.MarkFlagRequired("stack")
}
