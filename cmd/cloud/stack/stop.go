...
func init() {
	stopCmd.Flags().StringP("server", "S", "", "The server name to fetch the stack list from")
	stopCmd.MarkFlagRequired("server")

	stopCmd.Flags().StringP("stack", "s", "", "The stack to stop")
	stopCmd.MarkFlagRequired("stack")
}
